...
export class BolsaService {
  ...
  async getBolsaByCodigo(codigoBolsa: string): Promise<Bolsa> {
    try {
      return await this.bolsaRepository.findOne({
        where: {
          codigoBolsa
        }
      })
      ...
      throw new HttpException(
        `No se encontraro la bolsa ${codigoBolsa} en la base de datos.`,
